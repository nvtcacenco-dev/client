...
import { fetchCategory, fetchNewProducts } from "../../network/networkConfig";
...
export default function ItemBrowser({ products, imgURLs }: ItemBrowserProps) {
    ...
    const map = products.map((product, index) => (
        <AnimatePresence mode="wait" >
            ...
            >
                <Link
                    className='browsing-item-link h-100 w-100'
                ...
                <div className='d-flex item-description col-12 flex-column align-items-start row-gap-2'>
                    <p>{`$${parseFloat((product.Price as any)["$numberDecimal"]).toFixed(2)}`}</p>
                    <p>{product.Brand}</p>
                    <p>{product.Name}</p>
                </div>
            </motion.li>
    ...
    return (
        ...
        <motion.ul  className="browsing-item-list col-12 ">
